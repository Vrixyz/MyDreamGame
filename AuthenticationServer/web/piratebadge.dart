...
import 'package:server_code_lab/client/authenticationapi.dart';
...
Future main() async {
  ...
  _authApi = new AuthenticationApi(_client, rootUrl: '$protocol//$_serverUrl');
...
final auth = new GoogleOAuth2(
    "942079075130-01hs0upc7v40vg5jk8non8bj1jeffp21.apps.googleusercontent.com", // Client ID
    ["openid"],
    tokenLoaded:oauthReady);

Future oauthReady(Token token)async {
  print(token);
  print('and data:');
  print(token.data);
  print("over");
  ...
  l.provider = "google";
  l.data = token.data;
  //l.userId = token.userId;
  //l.data = token.data;
  //l.userId = token.userId;
  await _authApi.googleConnect(l).then((value) {
    print(value.account);
  }).catchError((err) {
    print(err);
  });
